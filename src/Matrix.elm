...
set x y newItem matrix =
    let
        insert indexedItem =
            if ( x, y ) == Tuple.first indexedItem then
                ( ( x, y ), newItem )
            else
                indexedItem
    in
...
quadCollapse matrix =
    let
        ...
        filter checkIndexes =
            List.filter
                (Tuple.first >> uncurry checkIndexes)
                matrix.data
...
map4 : (a -> b -> c -> d -> e) -> Matrix a -> Matrix b -> Matrix c -> Matrix d -> Maybe (Matrix e)
map4 f m1 m2 m3 m4 =
    let
...
sum matrix =
    List.foldl (Tuple.second >> (+)) 0 matrix.data
